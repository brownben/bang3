...
module!(iter, IterModule, {
  ...
  fn first() = |vm, iterator| {
    ...
    match result {
      Some(value) => Ok(vm.allocate_value(value, SOME_TYPE_ID)),
      None => Ok(Value::NONE)
    }
  };
  ...
  fn find() =  |vm, arg| {
    ...
    Ok(vm.allocate_value(NativeClosure::new("iter::find", func, arg), NATIVE_CLOSURE_TYPE_ID))
  };
  ...
  fn position() = |vm, arg| {
    ...
    Ok(vm.allocate_value(NativeClosure::new("iter::position", func, arg), NATIVE_CLOSURE_TYPE_ID))
  };
  ...
  fn toList() = |vm, iterator| {
    ...
    let mut list_location = vm.stash_object(list.as_ptr(), LIST_TYPE_ID)?;

    let mut state = 0;
    while let Some((value, new_state)) = iter_next(vm, iterator, state)? {
      let list = List::from(vm.pop_stashed_value(list_location).as_object());
      let updated_list = list.push(&mut vm.heap, value);
      list_location = vm.stash_object(updated_list.as_ptr(), LIST_TYPE_ID)?;
  ...
  fn reduce() = |vm, arg| {
    ...
    Ok(vm.allocate_value(NativeClosure::new("iter::reduce", func, arg), NATIVE_CLOSURE_TYPE_ID))
  };
  ...
  fn fold() = |vm, arg| {
    fn func_one(vm: &mut VM, a: Value, b: Value) -> Result<Value, ErrorKind> {
      ...
      let closure = NativeClosureTwo::new("iter::fold", func_two, a, b);
    ...
    Ok(vm.allocate_value(NativeClosure::new("iter::fold", func_one, arg), NATIVE_CLOSURE_TYPE_ID))
  };
  ...
  fn join() = |vm, arg| {
    ...
    Ok(vm.allocate_value(NativeClosure::new("iter::join", func, arg), NATIVE_CLOSURE_TYPE_ID))
  };
  ...
  ($name:literal $(#$infinite:ident)?, $next:expr) => {
    |vm, arg| {
      fn func(vm: &mut VM, arg: Value, iterator: Value) -> Result<Value, ErrorKind> {
        ...
        let iterator = IteratorTransform {
          iterator,
          arg,
          next: $next,
          length: function_transform!(infinite $($infinite)?, iterator, vm),
        };
        Ok(vm.allocate_value(iterator, ITERATOR_TRANSFORM_TYPE_ID))
      }
      ...
      let closure = NativeClosure::new(concat!("iter::", $name), func, arg);
